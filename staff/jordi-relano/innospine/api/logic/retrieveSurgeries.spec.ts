                        ...
                        Surgery.deleteMany()
                            .then(() =>
                                User.create({ name: 'equipo clavel', email: 'equipo@clavel.com', password: '1Z' })
                                    .then(user =>
                                        Promise.all([
                                            Surgery.create({ author: ObjectId, surgeryDate: '20/05/2024', name: 'kilombo',products:[], type: 'lumbar', hospital: 'quiron', note: 'kilombo does not works at all' }),
                                            Surgery.create({ author: user.id, surgeryDate: '22/05/2024', name: 'kilombo2', products: [], type: 'cervical', hospital: 'safa', note: 'kilombo is working properly' }),
                                            ...
                                            .then(([surgery1, surgery2]) =>
                                                logic.retrieveSurgeries(user.id)
                                                    .then(surgeries => {
                                                        ...
                                                        const surgery1b = surgeries.find(surgery => surgery.id === surgery1.id)
                                                        const surgery2b = surgeries.find(surgery => surgery.id === surgery2.id)
